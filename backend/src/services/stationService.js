...
const stationService = (stationCodes) => {
    try {
        const response = stationCodes.map((stationCode) => {
            const station = railwayStationJSON.features.find(
                item => item.properties.code === stationCode
            );
            ...
            if (!station.geometry?.coordinates) {
                throw new ApiError(
                    404,
                    `Coordinates not found: ${stationCode}`
                );
            }
        ...
        throw new ApiError(
            500,
            "Internal Server Error - stationService"
        );
    }
};
